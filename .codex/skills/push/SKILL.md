...
2. Run local validation before pushing.
   ...
   - If the issue, README, or repo scripts define a stronger validation command,
     run that too and record the result.
...
6. Write/update PR body explicitly using `.github/pull_request_template.md`
   when present; otherwise use a concise body with:
   - summary of the shipped change,
   - validation performed,
   - notable risks or follow-ups.
7. If PR already exists, refresh the body so it reflects the total current PR
...
# Prefer the repo template when present, otherwise write a compact fallback body.
